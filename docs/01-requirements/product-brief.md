...
|---|---|---|---|
| **R-01** | Holding customer funds in escrow may require CBN licensing. Concrete and severe, not theoretical: CBN's **PSSP** license (₦100M capital) explicitly cannot hold funds; the tier that explicitly *can* ("build escrow platforms for marketplace transactions") is the full **PSP** license, requiring **₦5 billion** in paid-up capital held in escrow with CBN — completely out of reach pre-launch. The open question is narrower but still unresolved: whether routing the hold through Monnify's Sub-Accounts (ADR-0001 — confirmed to exist) means CloseBuy itself never legally custodies funds, or whether CBN would still view CloseBuy as the party directing/holding them regardless of the technical mechanism. Public guidance doesn't settle this either way | **Severe — could block or reshape launch** | Get a Nigerian fintech/payments lawyer to confirm the Monnify sub-account structure specifically keeps CloseBuy outside PSP-tier licensing, before real customer money moves through the live platform (i.e., before M5, with enough lead time to restructure if the answer is no — e.g. making the vendor the merchant of record instead of CloseBuy). Cash-on-delivery is a separate, additional question (riders holding physical cash, not electronic funds) — see R-03, likely a smaller concern but not yet confirmed either |
