...
**Not built yet, deliberately** — Notifications and most of Admin exist only as empty directories matching architecture.md's module boundaries, or as `Placeholder` screens in the relevant app linking back to the story and milestone that will build them. **Admin is the next priority specifically** — right now nothing can move a vendor or rider application out of `pending`, which blocks the whole pipeline end to end.
