...
- **91 unit tests total now, up from 85** (`pnpm test`) — `pnpm typecheck && pnpm lint && pnpm build && pnpm test` all green across all 8 workspace packages. No screen in any of the four apps has a component/e2e test yet (see "Known rough edges"); verified instead by a real `next build` of all four apps and a dev-server smoke check of every new route.
