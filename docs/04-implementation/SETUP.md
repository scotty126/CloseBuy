...
- **85 unit tests total now, up from 66** (`pnpm test`) — `pnpm typecheck && pnpm lint && pnpm build && pnpm test` all green across all 8 workspace packages. The customer app's own screens aren't unit-tested (no screen in any of the four apps has been yet — see "Known rough edges"); verified instead by a real `next build` of all four apps and a dev-server smoke check of every new route.
