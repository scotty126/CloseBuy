...
- **Monnify** as the primary payment integration — chosen specifically for settlement speed: multi-daily payout to CloseBuy's own account vs. Paystack's next-business-day (and no weekend processing) default. That speed is meant to pass straight through to vendors as fast, reliable payout, which is a genuine product advantage and a real argument in the Founding Vendor pitch.
  ...
  **Confirmed, pre-M1 checkpoint closed:** Monnify has an equivalent Sub-Accounts / split-payment mechanism to Paystack's Subaccounts — a vendor's share of a payment can be held and released on the platform's instruction, rather than CloseBuy custodying it directly. This is necessary for R-01's mitigation to hold, but **not sufficient on its own** — see R-01 in the product brief for what's still genuinely unresolved (whether routing through Monnify's sub-accounts actually keeps CloseBuy outside CBN's fund-custody licensing requirement is a real legal question, not a technical one, and public guidance doesn't settle it either way).
