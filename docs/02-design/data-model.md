...
| collection_code | string, nullable | Set when the order reaches `READY_FOR_PICKUP`. Pickup: the customer reads it to the vendor at the counter. Delivery: the *vendor* reads it to the rider at handoff (US-C-07/US-R-04/US-V-06) — never exposed in any rider-facing API response (`dispatch/service.ts`'s `redactForRider`), or the rider could skip asking |
| delivery_code | string, nullable | Delivery orders only, set alongside `collection_code` at the same moment. The second handoff check (US-R-05): the customer reads *this* one to the rider at drop-off, confirming they're delivering to the right person, not just carrying the right order. Same reasoning as `collection_code` — never in a rider-facing response, and also stripped from vendor reads (the vendor has no part in this handoff) |
