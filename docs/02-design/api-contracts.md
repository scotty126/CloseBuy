...
| `GET /admin/metrics` — **built** | Platform health | US-A-07. Query `from`/`to` (inclusive `YYYY-MM-DD`, Africa/Lagos calendar days; default the last 30 days ending today; 422 `INVALID_RANGE` if reversed or over 366 days — the per-day series is built in application code from one row per order, so the range is capped). Returns orders (placed/fulfilled/in-flight/failed/cancelled-or-refunded/disputed), `grossMinor`, `completionRate`, `averageDeliveryMinutes` (+ `deliveriesMeasured`), `openDisputes`, active-vs-approved vendors and riders, and a zero-filled `perDay` series. **Definitions are part of the contract** (see `admin/metrics.ts`): everything is cohorted by placement day; "placed" excludes abandoned `PENDING_PAYMENT` checkouts; completion rate = fulfilled ÷ (fulfilled + cancelled/refunded + failed), so in-flight orders don't drag it down; gross excludes cancelled/refunded but does **not** net out goodwill/partial-dispute refunds (those deliberately leave `Order.status` alone); average delivery time is PAID→DELIVERED, delivery orders only, scheduled ones excluded. `openDisputes` and the *approved* counts are "right now", not range-scoped |
